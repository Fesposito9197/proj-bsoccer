...
@section('content')
    ...
    @if (!empty($player))
        <div id="show-container" class="container p-3 border border-3 border-white my-3 text-white">
            ...
            @endforeach
            {{-- bottone edit --}}
            <a href="{{ route('admin.players.edit', $player) }}" class="btn btn-warning"><i
                    class="fa-solid fa-pencil fa-lg fa-fw"></i></a>
            {{-- Bottone modale --}}
            <button type="button" class="btn btn-danger ms-2" data-bs-toggle="modal"
                data-bs-target="#modal-{{ $player->id }}">
                <i class="fa-solid fa-dumpster-fire fa-lg fa-fw"></i>
            </button>
            <!-- Modal -->
            <div class="modal fade" id="modal-{{ $player->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
    ...
    @else
        <div class="d-flex align-items-center text-light">
            ...
            <a href="{{ route('admin.players.create') }}" class="btn btn-success ms-2"><i
